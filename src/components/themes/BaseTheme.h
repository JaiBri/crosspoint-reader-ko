...
// Game2048 appended last: `None = 0` shifts every ordinal, and nothing
// serializes UIIcon to disk, so appending is safe.
enum UIIcon {
  None = 0,
  Folder,
  Text,
...
class BaseTheme {
 public:
  ...
  // `timeRemaining` is appended as slot 9, AFTER upstream's `fillMargin`:
  // StatusBarSettingsActivity already passes `false` positionally into slot 8,
  // so inserting ahead of it would misbind. Left non-virtual deliberately —
  // there is exactly one implementation and no theme overrides it.
  void drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage, const int pageCount,
                     std::string title, const int paddingBottom = 0, const int textYOffset = 0,
                     const bool fillMargin = true, const std::string& timeRemaining = "") const;
