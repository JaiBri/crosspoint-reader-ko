...
int HomeActivity::getMenuItemCount() const {
  int count = 5;  // File Browser, Recents, File transfer, 2048, Settings
...
void HomeActivity::loop() {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
    ...
    if (selectorIndex < recentBooks.size()) {
      ...
      onFileTransferOpen();
    } else if (menuSelectedIndex == game2048Idx) {
      onGame2048Open();
...
void HomeActivity::render(RenderLock&&) {
  ...
  std::vector<const char*> menuItems = {tr(STR_BROWSE_FILES), tr(STR_MENU_RECENT_BOOKS), tr(STR_FILE_TRANSFER),
                                        tr(STR_GAME_2048), tr(STR_SETTINGS_TITLE)};
  std::vector<UIIcon> menuIcons = {Folder, Recent, Transfer, Game2048, Settings};
